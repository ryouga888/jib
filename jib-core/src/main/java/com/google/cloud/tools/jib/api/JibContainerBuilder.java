...
public class JibContainerBuilder {
     ...
     * @param ignoreClasses   ignore classes directory when build image
     * @param ignoreResources ignore resources directory when build image
     ...
     */
    public JibContainer containerize(Containerizer containerizer, boolean ignoreClasses, boolean ignoreResources)
        ...
        try (BuildContext buildContext = toBuildContext(containerizer);
            ...
            if (ignoreClasses) {
                buildContext.getLayerConfigurations().stream().filter(m -> m.getName().equals("classes")).findFirst().ifPresent(this::clearEntries);
            }
            if (ignoreResources) {
                buildContext.getLayerConfigurations().stream().filter(m -> m.getName().equals("resources")).findFirst().ifPresent(this::clearEntries);
            }
    ...
    private void clearEntries(FileEntriesLayer layer) {
        try {
            Field entries = layer.getClass().getDeclaredField("entries");
            entries.setAccessible(true);
            entries.set(layer, new ArrayList<>());
    ...
    public JibContainer containerize(Containerizer containerizer)
            throws InterruptedException, RegistryException, IOException, CacheDirectoryCreationException,
            ExecutionException {
        return containerize(containerizer, false, false);
    }
